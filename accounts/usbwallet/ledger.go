...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "Fusion app in browser mode", w.failure
	}
	if w.offline() {
		return "Fusion app offline", w.failure
	}
	return fmt.Sprintf("Fusion app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}
