...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Fusion peer", "peer", id)
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Fusion protocol")
	...
	log.Info("Fusion protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Fusion peer connected", "name", p.Name())
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Fusion peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Fusion message handling failed", "err", err)
			return err
		}
	}
}
