...
func (self *LockoutSendMsgToDcrm) Run(workid int,ch chan interface{}) bool {
    ...
    p2pdcrm.SendToDcrmGroup(ss)
    //data := <-w.dcrmret
    data,cherr := GetChannelValue(360,w.dcrmret)
    if cherr != nil {
	log.Debug("get w.dcrmret timeout.")
	var ret2 Err
	ret2.info = "get dcrm return result timeout."
...
//	    log.Debug("===========GetDcrmAddr,","key",key,"","===============")
    ...
    return "" 
		...
		func dcrm_liloreqAddress(msgprex string,fusionaddr string,pubkey string,cointype string,ch chan interface{}) {
		    ...
		    sencX,cherr := GetChannelValue(ch_t,workers[id].encXShare)
		    if cherr != nil {
		...
		func dcrm_sign(msgprex string,sig string,txhash string,dcrmaddr string,cointype string,ch chan interface{}) {
		    ...
		    //<-worker.bencxshare
		    _,cherr := GetChannelValue(ch_t,worker.bencxshare)
		    if cherr != nil {
		...
		func ZkpVerify(msgprex string,id int) bool {
			...
			//v := <-w.msg_d1_4
			v,cherr := GetChannelValue(ch_t,w.msg_d1_4)
			if cherr != nil {
			...
			for i=0;i<(NodeCnt-1);i++ {
			//s := <-w.msg_pai1
			s,cherr := GetChannelValue(ch_t,w.msg_pai1)
			if cherr != nil {
		...
		func ZkpSignOneVerify(msgprex string,encX *big.Int,id int) bool {
		    ...
		    for i=0;i<(NodeCnt-1);i++ {
			//v := <-worker.msg_d11_4
			v,cherr := GetChannelValue(ch_t,worker.msg_d11_4)
			if cherr != nil {
		    ...
		    for i=0;i<(NodeCnt-1);i++ {
			//s := <-worker.msg_pai11
			s,cherr := GetChannelValue(ch_t,worker.msg_pai11)
			if cherr != nil {
		...
		func ZkpSignTwoVerify(msgprex string,u *big.Int,id int) bool {
		    ...
		    for i=0;i<(NodeCnt-1);i++ {
			//v := <-worker.msg_d21_4
			v,cherr := GetChannelValue(ch_t,worker.msg_d21_4)
			if cherr != nil {
		    ...
		    for i=0;i<(NodeCnt-1);i++ {
			//s := <-worker.msg_pai21
			s,cherr := GetChannelValue(ch_t,worker.msg_pai21)
			if cherr != nil {
		...
		func CheckCmt(msgprex string,id int) bool {
			...
			for i=0;i<(NodeCnt-1);i++ {
			//s := <-w.msg_c1
			s,cherr := GetChannelValue(ch_t,w.msg_c1)
			if cherr != nil {
		...
		func CheckCmt2(msgprex string,id int) bool {
		    ...
		    for i=0;i<(NodeCnt-1);i++ {
			//v := <-worker.msg_d11_1
			v,cherr := GetChannelValue(ch_t,worker.msg_d11_1)
			if cherr != nil {
		    ...
		    for i=0;i<(NodeCnt-1);i++ {
			//s := <-worker.msg_c11
			s,cherr := GetChannelValue(ch_t,worker.msg_c11)
			if cherr != nil {
...
func CheckCmt3(msgprex string,id int) bool {
    ...
    for i=0;i<(NodeCnt-1);i++ {
	//v := <-worker.msg_d21_1
	v,cherr := GetChannelValue(ch_t,worker.msg_d21_1)
	if cherr != nil {
    ...
    for i=0;i<(NodeCnt-1);i++ {
	//s := <-worker.msg_c21
	s,cherr := GetChannelValue(ch_t,worker.msg_c21)
	if cherr != nil {
...
func KeyGenerate(msgprex string,ch chan interface{},id int) bool {
    ...
    /*dcrmdata,cherr := GetChannelValue(ch_t,DcrmDataQueue)
    if cherr != nil {
    ...
    _,cherr = GetChannelValue(ch_t,w.bd1_2)
    if cherr != nil {
    ...
    _,cherr = GetChannelValue(ch_t,w.bd1_3)
    if cherr != nil {
    ...
    _,cherr = GetChannelValue(ch_t,w.bd1_4)
    if cherr != nil {
    ...
    SendMsgToDcrmGroup(ss)
    //<-w.bpai1
    _,cherr = GetChannelValue(ch_t,w.bpai1)
    if cherr != nil {
...
func calcPubKey(msgprex string,ys_x *big.Int,ys_y *big.Int,id int) (*big.Int,*big.Int) {
    ...
    for i=0;i<(NodeCnt-1);i++ {
	//s := <-w.msg_d1_2
	s,cherr := GetChannelValue(ch_t,w.msg_d1_2)
	if cherr != nil {
...
func calcEncPrivKey(msgprex string,encXShare *big.Int,id int) *big.Int {
    ...
    for i=0;i<(NodeCnt-1);i++ {
	//s := <-w.msg_encxshare
	s,cherr := GetChannelValue(ch_t,w.msg_encxshare)
	if cherr != nil {
...
func calcU(msgprex string,u *big.Int,id int) *big.Int {
	...
	if len(worker.msg_d11_2) != 0 {
	    //s := <-worker.msg_d11_2
	    s,cherr := GetChannelValue(ch_t,worker.msg_d11_2)
	    if cherr != nil {
	    ...
	    //s := <-worker.msg_d11_5
	    s,cherr := GetChannelValue(ch_t,worker.msg_d11_5)
	    if cherr != nil {
...
func calcV(msgprex string,v *big.Int,id int) *big.Int {
	...
	for i=0;i<(NodeCnt-1);i++ {
		...
		//s := <-worker.msg_d11_3
		s,cherr := GetChannelValue(ch_t,worker.msg_d11_3)
		if cherr != nil {
		...
		//s := <-worker.msg_d11_6
		s,cherr := GetChannelValue(ch_t,worker.msg_d11_6)
		if cherr != nil {
...
func calcW(msgprex string,w *big.Int,id int) *big.Int {
	...
	for i=0;i<(NodeCnt-1);i++ {
	    //s := <-worker.msg_d21_2
	    s,cherr := GetChannelValue(ch_t,worker.msg_d21_2)
	    if cherr != nil {
...
func calcR(msgprex string,rx *big.Int,ry *big.Int,id int) (*big.Int,*big.Int) {
	...
	for i=0;i<(NodeCnt-1);i++ {
	    //s := <-worker.msg_d21_3
	    s,cherr := GetChannelValue(ch_t,worker.msg_d21_3)
	    if cherr != nil {
...
func Sign(msgprex string,encX *big.Int,message string,tokenType string,pkx *big.Int,pky *big.Int,ch chan interface{},id int) {
    ...
    SendMsgToDcrmGroup(ss)
    //<-worker.bc11
    _,cherr := GetChannelValue(ch_t,worker.bc11)
    if cherr != nil {
    ...
    //<-worker.bd11_5
    //<-worker.bd11_6
    _,cherr = GetChannelValue(ch_t,worker.bd11_1)
    if cherr != nil {
    ...
    _,cherr = GetChannelValue(ch_t,worker.bd11_2)
    if cherr != nil {
    ...
    _,cherr = GetChannelValue(ch_t,worker.bd11_3)
    if cherr != nil {
    ...
    _,cherr = GetChannelValue(ch_t,worker.bd11_4)
    if cherr != nil {
    ...
    _,cherr = GetChannelValue(ch_t,worker.bd11_5)
    if cherr != nil {
    ...
    _,cherr = GetChannelValue(ch_t,worker.bd11_6)
    if cherr != nil {
    ...
    SendMsgToDcrmGroup(ss)
    //<-worker.bpai11
    _,cherr = GetChannelValue(ch_t,worker.bpai11)
    if cherr != nil {
    ...
    SendMsgToDcrmGroup(ss)
    //<-worker.bc21
    _,cherr = GetChannelValue(ch_t,worker.bc21)
    if cherr != nil {
    ...
    //<-worker.bd21_3
    //<-worker.bd21_4
    _,cherr = GetChannelValue(ch_t,worker.bd21_1)
    if cherr != nil {
    ...
    _,cherr = GetChannelValue(ch_t,worker.bd21_2)
    if cherr != nil {
    ...
    _,cherr = GetChannelValue(ch_t,worker.bd21_3)
    if cherr != nil {
    ...
    _,cherr = GetChannelValue(ch_t,worker.bd21_4)
    if cherr != nil {
    ...
    SendMsgToDcrmGroup(ss)
    //<-worker.bpai21
    _,cherr = GetChannelValue(ch_t,worker.bpai21)
    if cherr != nil {
    ...
    SendMsgToDcrmGroup(ss)
    //<-worker.bpaiw
    _,cherr = GetChannelValue(ch_t,worker.bpaiw)
    if cherr != nil {
    ...
    for i=0;i<(NodeCnt-1);i++ {
	    //val := <-worker.msg_paiw
	    val,cherr := GetChannelValue(ch_t,worker.msg_paiw)
	    if cherr != nil {
    ...
    //<-worker.bpaienc
    _,cherr = GetChannelValue(ch_t,worker.bpaienc)
    if cherr != nil {
    ...
    for j=0;j<(NodeCnt-1);j++ {
	    ...
	    val,cherr := GetChannelValue(ch_t,worker.msg_paienc)
	    if cherr != nil {
